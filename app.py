...
TREND_ONE_WAY_MODE = int(os.getenv("TREND_ONE_WAY_MODE", "1"))
RANGE_BOTH_SIDES   = int(os.getenv("RANGE_BOTH_SIDES", "1"))
...
SL_TRIGGER_MODE = os.getenv("SL_TRIGGER_MODE", "wick").lower()  # close|wick
...
ENTRY_LEVEL_COOLDOWN_SEC = int(os.getenv("ENTRY_LEVEL_COOLDOWN_SEC", "300"))
LEVEL_PRICE_ROUND_DP = int(os.getenv("LEVEL_PRICE_ROUND_DP", "2"))
...
def _round_level(px):
    return round(float(px), LEVEL_PRICE_ROUND_DP)

def _level_key(side, px):
    return f"{side}:{_round_level(px)}"

def _cleanup_recent_level_entries(now_ms):
    ttl_ms = max(ENTRY_LEVEL_COOLDOWN_SEC, 0) * 1000
    if ttl_ms <= 0:
        return
    stale = [k for k, ts in recent_level_entries.items() if (now_ms - ts) > ttl_ms]
    for k in stale:
        recent_level_entries.pop(k, None)

def _entry_recently_used(side, px, now_ms):
    ttl_ms = max(ENTRY_LEVEL_COOLDOWN_SEC, 0) * 1000
    if ttl_ms <= 0:
        return False
    ts = recent_level_entries.get(_level_key(side, px))
    return ts is not None and (now_ms - ts) < ttl_ms

def _mark_entry_used(side, px, now_ms):
    recent_level_entries[_level_key(side, px)] = now_ms

def current_open_notional(side=None):
    if side == "long":
        return sum(e["qty"] * e["entry"] for e in paper["open_entries"])
    if side == "short":
        return sum(e["qty"] * e["entry"] for e in paper["short_entries"])
    return current_open_notional("long") + current_open_notional("short")

def _can_add_notional(side, add_usdt):
    long_now = current_open_notional("long")
    short_now = current_open_notional("short")
    total_now = long_now + short_now
    if total_now + add_usdt > MAX_OPEN_NOTIONAL:
        return False
    if side == "long" and long_now + add_usdt > MAX_NOTIONAL_LONG:
        return False
    if side == "short" and short_now + add_usdt > MAX_NOTIONAL_SHORT:
        return False
    return True

def current_mark_price():
    try:
        v = runtime.get("last_close")
        if v is None:
            return None
        return float(v)
    except Exception:
...
def compute_paper_metrics(mark_price=None):
    if mark_price is None:
        mark_price = current_mark_price()

    long_notional = sum(e["qty"] * e["entry"] for e in paper["open_entries"])
    short_notional = sum(e["qty"] * e["entry"] for e in paper["short_entries"])

    if mark_price is None or mark_price <= 0:
        return {
            "mark_price": None,
            "open_long_notional_usdt": round(long_notional, 6),
            "open_short_notional_usdt": round(short_notional, 6),
            "unrealized_pnl_usdt": None,
            "estimated_equity_usdt": None,
        }

    long_unreal = 0.0
    long_liq_value = 0.0
    for e in paper["open_entries"]:
        proceeds_now = e["qty"] * mark_price
        est_close_fee = proceeds_now * PAPER_FEE_PCT
        long_unreal += proceeds_now - est_close_fee - (e["qty"] * e["entry"]) - e["fee_usdt"]
        long_liq_value += proceeds_now - est_close_fee

    short_unreal = 0.0
    for e in paper["short_entries"]:
        cost_cover_now = e["qty"] * mark_price
        est_close_fee = cost_cover_now * PAPER_FEE_PCT
        short_unreal += (e["qty"] * e["entry"]) - cost_cover_now - e["fee_usdt"] - est_close_fee

    unrealized = long_unreal + short_unreal
    est_equity = paper["usdt_trading"] + paper["usdt_sparen"] + long_liq_value + short_unreal
    ...
    return {
        ...
        "estimated_equity_usdt": round(est_equity, 6),
    }

def place_ladder_spread(ex, side_levels, base_usdt, max_per_tick, spacing, center_adj, atr_val, now_ms):
    """
    ...
    """
    placed = 0
    _cleanup_recent_level_entries(now_ms)
    for px, side in side_levels:
        ...
        if not _notional_ok(px, qty):
            continue
        if _entry_recently_used(side, px, now_ms):
            continue
        if DRY_RUN and not _can_add_notional(side, base_usdt):
            continue
        try:
            if DRY_RUN:
                # PAPER — doe exact wat je al had: entry + TP
                if side == "long":
                    lot = paper_place_buy(px, now_ms)
                    if lot:
                        paper_place_tp_long(lot, spacing, center_adj)
                        paper_place_sl_long(lot, atr_val)
                else:
                    lot = paper_place_short(px, now_ms)
                    if lot:
                        paper_place_tp_short(lot, spacing, center_adj)
                        paper_place_sl_short(lot, atr_val)
            else:
            ...
            placed += 1
            _mark_entry_used(side, px, now_ms)
            _sleep_throttle()
...
paper = {
    ...
    "open_orders": [],                  # [{'id','type':'tp|sl','side','price','qty','status','lot_id'}]
    "closed_trades": []
}
...
def build_grids(center, atr_val, mode, total_layers, bias=None):
    ...
    if mode == "range" and not RANGE_BOTH_SIDES:
        long_layers = total_layers
        short_layers = 0

    if bias == "bearish":
        center_adj = center - (atr_val * BIAS_SHIFT_ATR)
        if TREND_ONE_WAY_MODE:
            short_layers = total_layers
            long_layers = 0
        else:
...
def make_lot_id(side):
    return f"{side}-{int(time.time()*1000)}-{int(time.perf_counter_ns()%1_000_000)}"

# ===== Paper helpers (lot-based + sparen) =====
def paper_reset():
    recent_level_entries.clear()
    paper.update({
        "usdt_trading": PAPER_USDT_START,
        "usdt_sparen":  0.0,
        "xrp": 0.0,
...
def paper_place_buy(price, now_ts):
    ...
    if paper["usdt_trading"] < usdt:
        return None
    qty = round(usdt / max(price, 1e-12), 6)
    fee_buy = usdt * PAPER_FEE_PCT
    lot_id = make_lot_id("long")
    paper["usdt_trading"] -= (usdt + fee_buy)
    paper["xrp"] += qty
    lot = {"id": lot_id, "qty": qty, "entry": price, "fee_usdt": fee_buy, "ts": now_ts}
    paper["open_entries"].append(lot)
    if NOTIFY_FILLS:
        send_telegram(f"🧪 PAPER BUY {qty:g} @ {price:.6f} (fee {fee_buy:.4f} USDT)")
    return lot

def paper_place_tp_long(lot, spacing, center):
    tp = compute_tp_price(TP_MODE, lot["entry"], spacing, center, "long")
    oid = f"PTP-L-{lot['id']}"
    paper["open_orders"].append({
        "id": oid, "type": "tp", "side": "sell", "price": tp, "qty": lot["qty"],
        "status": "open", "lot_id": lot["id"]
    })
    if NOTIFY_TP_PLACED:
        send_telegram(f"🧪 PAPER TP geplaatst SELL @ {tp:.6f}")

def paper_place_sl_long(lot, atr):
    sl = compute_sl_price(lot["entry"], atr, "long")
    oid = f"PSL-L-{lot['id']}"
    paper["open_orders"].append({
        "id": oid, "type": "sl", "side": "sell", "price": sl, "qty": lot["qty"],
        "status": "open", "lot_id": lot["id"]
    })
...
def paper_place_short(price, now_ts):
    ...
    lot_id = make_lot_id("short")
    paper["usdt_trading"] -= fee_open
    lot = {"id": lot_id, "qty": qty, "entry": price, "fee_usdt": fee_open, "ts": now_ts}
    paper["short_entries"].append(lot)
    if NOTIFY_FILLS:
        send_telegram(f"🧪 PAPER SHORT {qty:g} @ {price:.6f} (fee {fee_open:.4f} USDT)")
    return lot

def paper_place_tp_short(lot, spacing, center):
    tp = compute_tp_price(TP_MODE, lot["entry"], spacing, center, "short")
    oid = f"PTP-S-{lot['id']}"
    paper["open_orders"].append({
        "id": oid, "type": "tp", "side": "buy", "price": tp, "qty": lot["qty"],
        "status": "open", "lot_id": lot["id"]
    })
    if NOTIFY_TP_PLACED:
        send_telegram(f"🧪 PAPER TP geplaatst BUY @ {tp:.6f} (short)")

def paper_place_sl_short(lot, atr):
    sl = compute_sl_price(lot["entry"], atr, "short")
    oid = f"PSL-S-{lot['id']}"
    paper["open_orders"].append({
        "id": oid, "type": "sl", "side": "buy", "price": sl, "qty": lot["qty"],
        "status": "open", "lot_id": lot["id"]
    })

def _take_specific_lot(lots_key, lot_id, qty_need):
    for idx, e in enumerate(paper[lots_key]):
        if e.get("id") != lot_id:
            continue
        q = min(e["qty"], qty_need)
        part_fee = e["fee_usdt"] * (q / e["qty"])
        taken = [{"id": e["id"], "qty": q, "entry": e["entry"], "fee_usdt": part_fee, "ts": e["ts"]}]
        e["qty"] -= q
        e["fee_usdt"] -= part_fee
        if e["qty"] <= 1e-9:
            paper[lots_key].pop(idx)
        return taken, q
    return [], 0.0

def _cancel_linked_orders(lot_id, exclude_id=None):
    for o in paper["open_orders"]:
        if o.get("lot_id") == lot_id and o.get("status") == "open" and o.get("id") != exclude_id:
            o["status"] = "cancelled"

def _record_paper_close(side, reason, avg_entry, exit_price, filled, pnl, ts_entry, now_ts):
...
def _maybe_move_to_sparen(pnl):
    if (
        SPAREN_ENABLED
        and pnl > SPAREN_MIN_PNL_USDT
        and SPAREN_SPLIT_PCT > 0
        and paper["usdt_trading"] > 0
    ):
        to_save = min(pnl * (SPAREN_SPLIT_PCT / 100.0), max(paper["usdt_trading"], 0.0))
        if to_save <= 0:
            return
...
def _close_long_order(order, now_ts, reason):
    qty = order["qty"]
    price = order["price"]
    parts, filled = _take_specific_lot("open_entries", order.get("lot_id"), qty)
    order["status"] = "filled"
    _cancel_linked_orders(order.get("lot_id"), exclude_id=order.get("id"))
    if filled <= 0:
        return
    ...
    _record_paper_close("long", reason, avg_entry, price, filled, pnl, parts[0]["ts"] if parts else None, now_ts)

    if NOTIFY_TP_SL:
        label = "CLOSE LONG" if reason == "tp" else "STOP LONG"
        send_telegram(
            f"🧪 {label} {filled:g} @ {price:.6f} "
            f"(entry {avg_entry:.6f}) | PnL {pnl:+.4f} USDT"
        )

    _maybe_move_to_sparen(pnl)
    _maybe_send_sell_overview()

def _close_short_order(order, now_ts, reason):
    qty = order["qty"]
    price = order["price"]
    parts, filled = _take_specific_lot("short_entries", order.get("lot_id"), qty)
    order["status"] = "filled"
    _cancel_linked_orders(order.get("lot_id"), exclude_id=order.get("id"))
    if filled <= 0:
        return
    ...
    paper["realized_pnl_usdt"] += pnl
    _record_paper_close("short", reason, avg_entry, price, filled, pnl, parts[0]["ts"] if parts else None, now_ts)

    if NOTIFY_TP_SL:
        label = "COVER SHORT" if reason == "tp" else "STOP SHORT"
        send_telegram(
            f"🧪 {label} {filled:g} @ {price:.6f} "
            f"(entry {avg_entry:.6f}) | PnL {pnl:+.4f} USDT"
        )
...
def paper_exec_orders(tp_long_price, tp_short_price, sl_long_price, sl_short_price, now_ts):
    eps_up = 1.0 + PAPER_EPS_PCT
    eps_dn = 1.0 - PAPER_EPS_PCT

    # Eerst SL, daarna TP. Dat is conservatiever als beide in dezelfde candle geraakt worden.
    for reason in ("sl", "tp"):
        for o in list(paper["open_orders"]):
            if o.get("status") != "open" or o.get("type") != reason:
                continue

            if o["side"] == "sell":  # long sluiten
                if reason == "sl":
                    triggered = sl_long_price <= o["price"] * eps_dn
                else:
                    triggered = tp_long_price >= o["price"] * eps_up
                if triggered:
                    _close_long_order(o, now_ts, reason)
            elif o["side"] == "buy":  # short sluiten
                if reason == "sl":
                    triggered = sl_short_price >= o["price"] * eps_up
                else:
                    triggered = tp_short_price <= o["price"] * eps_dn
                if triggered:
                    _close_short_order(o, now_ts, reason)
...
def daily_report_text(ex):
    ...
    if paper["enabled"]:
        ...
        lines.append(
            f"(Paper) Trading≈{paper['usdt_trading']:.2f} | Sparen≈{paper['usdt_sparen']:.2f} | "
            f"XRP(long)≈{paper['xrp']:.4f} | SHORT_qty≈{short_qty:.4f} | Realized≈{paper['realized_pnl_usdt']:.2f}"
        )
        if m.get("estimated_equity_usdt") is not None:
            lines.append(
                f"(Paper) Equity≈{m['estimated_equity_usdt']:.2f} | Unrealized≈{m['unrealized_pnl_usdt']:.2f} | Mark≈{m['mark_price']:.2f}"
            )
...
def paper_summary_dict():
    ...
    return {
        ...
        "open_long_notional_usdt": metrics.get("open_long_notional_usdt"),
        "open_short_notional_usdt": metrics.get("open_short_notional_usdt"),
        "unrealized_pnl_usdt": metrics.get("unrealized_pnl_usdt"),
        "estimated_equity_usdt": metrics.get("estimated_equity_usdt"),
...
def build_closed_trades_csv():
    ...
    w.writerow([])
    w.writerow(["side","reason","entry","exit","qty","pnl_usdt","ts_entry","ts_exit"])
    for t in paper["closed_trades"]:
        w.writerow([
            t.get("side"), t.get("reason"), t.get("entry"), t.get("exit"), t.get("qty"), t.get("pnl_usdt"),
...
def send_paper_snapshot_to_telegram():
    ...
    msg = (
        ...
        f"Unrealized PnL: <b>{s['unrealized_pnl_usdt'] if s['unrealized_pnl_usdt'] is not None else 'n/a'}</b> USDT\n"
        f"Equity: <b>{s['estimated_equity_usdt'] if s['estimated_equity_usdt'] is not None else 'n/a'}</b> USDT\n"
...
# ===== Bot thread =====
def bot_thread():
    global last_trade_ms, last_paper_snapshot_min, last_candle_key
    ...
    while not _stop.is_set():
        try:
            ...
            candle_key = str(d["ts"].iloc[-1])
            if candle_key != last_candle_key:
                last_candle_key = candle_key
                # laat oude levels pas na cooldown opnieuw toe; geen per-candle spam meer
                _cleanup_recent_level_entries(int(time.time() * 1000))
            ...
            if need_rebuild:
                ...
                if NOTIFY_REBUILD:
                    ...
                    if bias == "bearish":
                        if TREND_ONE_WAY_MODE:
                            msg += "\n🧭 Bias: <b>bearish</b> (alleen shorts + center↓)"
                        else:
                            msg += "\n🧭 Bias: <b>bearish</b> (meer shorts + center↓)"
                    elif bias == "bullish":
                        if TREND_ONE_WAY_MODE:
                            msg += "\n🧭 Bias: <b>bullish</b> (alleen longs + center↑)"
                        else:
            ...
            if paper["enabled"]:
                ...
                _ = place_ladder_spread(
                    ex, long_levels, BASE_ORDER_USDT, MAX_NEW_ORDERS_PER_TICK, spacing, center_adj, atr, now_ms
                )
                _ = place_ladder_spread(
                    ex, short_levels, BASE_ORDER_USDT, MAX_NEW_ORDERS_PER_TICK, spacing, center_adj, atr, now_ms
                )

                tp_trigger_long = hi if TP_TRIGGER_MODE == "wick" else close
                tp_trigger_short = lo if TP_TRIGGER_MODE == "wick" else close
                sl_trigger_long = lo if SL_TRIGGER_MODE == "wick" else close
                sl_trigger_short = hi if SL_TRIGGER_MODE == "wick" else close
                paper_exec_orders(
                    tp_trigger_long,
                    tp_trigger_short,
                    sl_trigger_long,
            ...
            # === Runtime + rapporten ===
            long_notional = current_open_notional("long")
            short_notional = current_open_notional("short")
            runtime.update({
                ...
                "global_notional": round(long_notional + short_notional, 6)
            })
...
def report_table():
    ...
    try:
        ...
        global paper
        rows = paper.get("closed_trades", []) if isinstance(paper, dict) else []
    ...
    html = (
        ...
        "<table border='1' cellpadding='6' cellspacing='0'>"
        "<tr>"
        "<th>side</th><th>reason</th><th>ts_entry</th><th>ts_exit</th>"
